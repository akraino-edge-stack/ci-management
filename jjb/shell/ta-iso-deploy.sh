...
if [ -n "${ROOT_BUILD_CAUSE_GIT_COMMIT}" ]; then
    echo "RPM - Looking up gerrit change details for commit ${ROOT_BUILD_CAUSE_GIT_COMMIT}"
    FQ="[.messages[] | select(.author.email | contains('${ROOT_BUILD_CAUSE_ACCOUNT_EMAIL:-}'))][-1] | .date"
    set +e
    ROOT_BUILD_TIMESTAMP=$(curl -sL "${GERRIT_URL}/changes/${ROOT_BUILD_CAUSE_GIT_COMMIT}/detail" | \
