...
portal_user_interface)
    ...
    XMLFILE="https://nexus.akraino.org/service/local/repositories/snapshots/content/org/akraino/${PROJECT}/${VERSION}/maven-metadata.xml"
    curl -O "${XMLFILE}"
    V2=`grep value maven-metadata.xml | sed -e 's;</value>;;' -e 's;.*<value>;;' | uniq`
    WARFILE="https://nexus.akraino.org/service/local/repositories/snapshots/content/org/akraino/${PROJECT}/${VERSION}/${PROJECT}-${V2}.war"
    curl -O "${WARFILE}"
...
camunda_workflow)
    ...
    curl -O "${XMLFILE}"
    V2=`grep value maven-metadata.xml | sed -e 's;</value>;;' -e 's;.*<value>;;' | uniq`
    JARFILE="https://nexus.akraino.org/service/local/repositories/snapshots/content/org/akraino/${PROJECT}/${VERSION}/${PROJECT}-${V2}.jar"
    curl -O ${JARFILE}
    ;;
...
postgres_db_schema)
    ...
    XMLFILE="https://nexus.akraino.org/service/local/repositories/snapshots/content/org/akraino/yaml_builds/yaml_builds/${VERSION}/maven-metadata.xml"
    curl -O "${XMLFILE}"
    V2=`grep value maven-metadata.xml | sed -e 's;</value>;;' -e 's;.*<value>;;' | uniq`
    TGZFILE="https://nexus.akraino.org/service/local/repositories/snapshots/content/org/akraino/yaml_builds/yaml_builds/${VERSION}/yaml_builds-${V2}.tgz"
    curl -O "${TGZFILE}"
