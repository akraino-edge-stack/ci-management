...
then
    ...
    then
        python3 validation/bluval/blucon.py -o -l "$blueprint_layer" "$blueprint_name"
    else
        python3 validation/bluval/blucon.py -l "$blueprint_layer" "$blueprint_name"
    fi
else
    if [ "$is_optional" == "true" ] || [ "$OPTIONAL" == "yes" ]
    then
        python3 validation/bluval/blucon.py -o "$blueprint_name"
    else
